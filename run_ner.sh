...
perform_trial(){
  ...
  tmux send -t trials_ner:"$file_name" "conda activate kgrl;
  python -m kgrl trial $config_file -hls -mg -ci --use-best --opt-storage sqlite:///optuna.db --suffix rw_ner -fp n_episodes 1 -fp deterministic True" ENTER
...
tmux new-session -d -s trials_ner -n htop htop
for f in $Files/*ner.yml
do
  echo $f
  perform_trial "$f"
done