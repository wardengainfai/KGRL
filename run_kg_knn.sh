...
perform_trial(){
  ...
  tmux send -t trials_knn:"$file_name" "conda activate kgrl;
  python -m kgrl trial $config_file -hls -mg -ci --use-best --opt-storage sqlite:///optuna.db --suffix knn -fp n_episodes 1 -fp deterministic True" ENTER
...
tmux new-session -d -s trials_knn -n htop htop
for f in $Files/*knn.yml
do
  echo $f
  perform_trial $f
done